...
# --- הגדרות מערכת (אלוקים יעזור לנו לסיים את זה הפעם) ---
...
def get_short_link(raw_url):
    """קיצור קישור יציב - המתנה של 2.2 שניות מבטיחה s.click"""
    try:
...
def search_aliexpress(keyword, offset=0):
    """מנוע חיפוש פרימיום עם שכל וסינון דינמי"""
    try:
        en_keyword = GoogleTranslator(source='auto', target='en').translate(keyword).lower()
        
        # סינון מחיר ואיכות - רק למוצרים שדורשים זאת
        min_price = "0"
        bad_words = ['adapter', 'cable', 'mount', 'rear view', 'borescope', 'parts']
        
        # הזרקת איכות וביטול סינונים אם המשתמש חיפש את המילה בעצמו
        if any(w in en_keyword for w in ['camera', 'dash', 'car', 'dvr']):
            en_keyword = f"70mai DDPai Dash Cam 4K GPS {en_keyword}"
            min_price = "60"
        elif any(w in en_keyword for w in ['sticker', 'label', 'decal', 'custom', 'cover']):
            bad_words = [bw for bw in bad_words if bw not in en_keyword] # מבטל חסימה על מה שחיפשת
            min_price = "0"
        ...
        for p in products_raw:
            ...
            # בחיפוש זול (כמו מדבקות) נהיה פחות קשוחים עם הדירוג
...
def draw_elite_number(draw, cx, cy, num):
    ...
    if num == 1: draw.rectangle([bx+2, by, bx+6, by+20], fill="black")
        ...
        for r in [[0,0,10,3],[8,0,10,12],[0,10,10,13],[0,12,3,20],[0,17,10,20]]:
...
def handle_message(message):
    try:
        ...
        loading = bot.send_message(chat_id, f"🔍 <b>סורק פריטי פרימיום עבור: '{search_query}'...</b>", parse_mode="HTML")
        products = search_aliexpress(search_query)
        
        if not products:
            bot.edit_message_text("לא נמצאו תוצאות איכותיות. נסו חיפוש שונה.", chat_id, loading.message_id)
            return
...
# --- ניקוי בוטים קודמים למניעת שגיאת 409 ---
bot.remove_webhook()
