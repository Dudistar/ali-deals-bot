...
def get_short_link(raw_url):
    """קיצור קישור רשמי להבטחת יציבות בטלגרם"""
    try:
        time.sleep(1.2)
...
def search_aliexpress(keyword):
    """שליפת מוצרים עם המרת דירוג וסינון אביזרים נלווים"""
    try:
        ...
        for p in products_raw:
            ...
            # סינון אביזרים אם לא חיפשת אותם במפורש
            if any(bad in title_raw.lower() for bad in ['case','cover','adapter','cable']) and \
               not any(k in en_keyword for k in ['case','cover','adapter','cable']):
                continue

            try: title_he = GoogleTranslator(source='auto', target='iw').translate(title_raw)
            except: title_he = title_raw
            
            # קיצור כותרת למניעת חריגת תווים בטלגרם
            if len(title_he) > 55: title_he = title_he[:52] + "..."

            # המרת דירוג מאחוזים לציון 1-5
            try:
                raw_rate = str(p.get('evaluate_rate', '95')).replace('%', '')
                val = float(raw_rate)
                star_rating = round(val / 20, 1) if val > 5 else round(val, 1)
            except:
                star_rating = 4.8
...
def create_collage(image_urls):
    """יצירת קולאז' עם מספרים בולטים וגדולים בעיגולים צהובים"""
    images = []
    sq = 500
    ...
    # טעינת פונט עבור Railway (DejaVuSans בולט וקיים ברוב שרתי הלינוקס)
    try:
    ...
    for i, img in enumerate(images):
        ...
        draw.ellipse((cx, cy, cx+160, cy+160), fill="#FFD700", outline="black", width=8)
        # כתיבת המספר - גדול וממורכז
...
def handle_message(message):
    try:
        ...
        loading = bot.send_message(message.chat.id, f"🔎 מחפש עבורכם את הדילים הטובים ביותר ל-'{search_query}'...")
        
        products = search_aliexpress(search_query)
        if not products:
            bot.edit_message_text("לא נמצאו תוצאות מדויקות מספיק. נסו חיפוש אחר.", message.chat.id, loading.message_id)
            return
        ...
        bot.send_message(message.chat.id, "אירעה תקלה זמנית, אנא נסו שוב.")
