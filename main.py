...
# הגדרת סשן עם מנגנון Retry למניעת קריסות רשת
...
def smart_query_optimizer(user_text):
    ...
    logging.info(f"Optimizing: {user_text}")
    ...
    if HAS_GEMINI:
        try:
            ...
            4. If user specifies color (e.g. "White"), INCLUDE it.
            ...
            logging.error(f"AI Opt Error: {e}")
...
# ==========================================
# 🎣 שלב 2: הרשת (API Fetcher) - יציב!
# ==========================================
def get_ali_products(cleaned_query):
    # חזרנו ל-50 מוצרים כדי למנוע ניתוקים מהשרת
    params = {
...
def filter_candidates(products, query_en):
    ...
    STRICT RULES:
    1. ACCESSORY TRAP: If user wants "Buds", "Phone", "Drone" -> REJECT "Case", "Strap", "Silicone", "Cover".
    2. PRICE REALISM: 
       - If user wants Electronics (Buds/Phone/Watch) and price is < 30 ILS ($8), IT IS FAKE/ACCESSORY -> REJECT.
       - A Case costs $5. Buds cost $50+. Know the difference.
    3. RELEVANCE: Reject unrelated items.
...
# ==========================================
# 🛠️ כלי עזר (Link, Image, Translate)
# ==========================================
...
def start(m):
    welcome_msg = "✨ <b>ברוכים הבאים ל-DrDeals Premium</b> 💎\nכתבו <b>'חפש לי'</b> ואת שם המוצר."
    try:
        if os.path.exists('welcome.jpg'):
            with open('welcome.jpg', 'rb') as p:
                bot.send_photo(m.chat.id, p, caption=welcome_msg, parse_mode="HTML")
        else:
...
def handle_text(m):
    ...
    # 3. סינון
    bot.edit_message_text(f"💎 <b>מסנן איכות...</b>", m.chat.id, msg.message_id, parse_mode="HTML")
    final_list = filter_candidates(products, query_en)
    
    bot.delete_message(m.chat.id, msg.message_id)

    if not final_list:
        bot.send_message(m.chat.id, "🤔 מצאתי רק אביזרים/חלקים, לא את המוצר עצמו.")
        return
    ...
    bot.send_message(m.chat.id, text_lines + "💎 <b>DrDeals Selection</b>", parse_mode="HTML", reply_markup=markup, disable_web_page_preview=True)
