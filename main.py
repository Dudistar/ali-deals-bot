...
def get_short_link(raw_url):
    try:
        time.sleep(1.2)  # השהייה למניעת ApiCallLimit
        ...
        res = resp.get('aliexpress_affiliate_link_generate_response', {})\
                  ...
                  .get('promotion_link', [])
        if res:
            return res[0].get('promotion_link') or res[0].get('promotion_short_link') or raw_url
    except Exception as e:
        print("Error get_short_link:", e)
    return raw_url

def search_aliexpress(keyword):
    """שליפת 4 מוצרים עם קישור קצר, תמונה, מחיר ודירוג"""
    try:
        try:
            en_keyword = GoogleTranslator(source='auto', target='en').translate(keyword)
        except:
            en_keyword = keyword
        ...
        for p in products_raw:
            ...
            if any(bad in title_raw.lower() for bad in ['case','cover','part','accessory']): continue
            try:
                title_he = GoogleTranslator(source='auto', target='iw').translate(title_raw)
            except:
                title_he = title_raw
            results.append({
                "title": title_he,
        ...
        print("Error search_aliexpress:", e)
        return None
...
def create_collage(image_urls):
    ...
    for url in image_urls:
        try:
            ...
            images.append(Image.new('RGB',(sq,sq),color='#EEEEEE'))
    while len(images)<4:
        images.append(Image.new('RGB',(sq,sq),color='#EEEEEE'))
...
def handle_message(message):
    try:
        ...
        loading = bot.send_message(message.chat.id, f"🔎 מחפש עבורך את הדילים הטובים ביותר ל'{search_query}'...")
        
        products = search_aliexpress(search_query)
        if not products:
            bot.edit_message_text("מצטער, לא נמצאו תוצאות מתאימות כרגע.", message.chat.id, loading.message_id)
            return
        ...
        # שליחת הקולאז'
        bot.send_photo(message.chat.id, collage, caption=f"🎯 מצאתי את המוצרים המומלצים עבור: <b>{search_query}</b>", parse_mode="HTML")
        
        # הודעת טקסט עם קישורים + כפתורים
        ...
        for i,p in enumerate(products):
            ...
            text_msg += f"🔥 <b>-{p['discount']}</b> | 💰 מחיר: <b>{p['price']}₪</b>\n"
            text_msg += f"⭐ דירוג: {p['rating']} | 🛒 רכישות: {p['orders']}\n"
        ...
        bot.send_message(message.chat.id, "אירעה תקלה בעיבוד הנתונים, נסו שנית.")

print("Bot is LIVE – אמיתי ומוכן!")
